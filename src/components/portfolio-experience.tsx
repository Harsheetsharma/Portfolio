...
export function PortfolioExperience() {
  ...
  return (
    <SmoothScroll>
      <CustomCursor />
      <motion.div className="fixed left-0 top-0 z-[80] h-1 origin-left bg-gradient-to-r from-cyan-300 via-fuchsia-300 to-emerald-300" style={{ scaleX: progress }} />

      <AnimateLoader loaded={loaded} />

      <div ref={rootRef} className="relative min-h-screen overflow-hidden bg-[#050509] text-white">
        ...
        <main id="top" className="relative z-10">
          <section className="relative flex min-h-screen items-center px-4 pb-24 pt-32 sm:px-6 lg:px-8">
            <motion.div style={{ y: heroY, scale: heroScale }} className="mx-auto grid w-full max-w-7xl gap-12 lg:grid-cols-[1.02fr_0.98fr] lg:items-center">
              <div>
                <motion.div
                ...
                <motion.h1
                  ...
                  animate={{ opacity: loaded ? 1 : 0, y: loaded ? 0 : 46, filter: loaded ? "blur(0px)" : "blur(20px)" }}
          ...
          <SectionReveal id="about" className="mx-auto grid max-w-7xl gap-10 px-4 py-28 sm:px-6 lg:grid-cols-[0.8fr_1.2fr] lg:px-8">
            <div>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {metrics.map((metric, index) => (
                  ...
                  >
                    ...
                    <span className="mt-3 block text-sm leading-6 text-white/50">{metric.label}</span>
                  </motion.article>
                ))}
              </div>
            </div>
          ...
          <section id="projects" className="horizontal-pin relative min-h-screen overflow-hidden py-28">
            <div className="mx-auto mb-12 max-w-7xl px-4 sm:px-6 lg:px-8">
              <p className="eyebrow">Featured Projects</p>
              <h2 className="section-title mt-5 max-w-4xl">Cinematic cards for backend work with real engineering weight.</h2>
            </div>
            <div ref={horizontalRef} className="flex w-max gap-6 px-4 sm:px-6 lg:px-8">
          ...
          <SectionReveal id="skills" className="mx-auto max-w-7xl px-4 py-28 sm:px-6 lg:px-8">
            <div className="mb-12 flex flex-col justify-between gap-6 md:flex-row md:items-end">
              <div>
                <p className="eyebrow">Skills</p>
                <h2 className="section-title mt-5">Interactive stack, built for modern backend products.</h2>
              </div>
                ...
                >
                  ...
                  <div className="mt-6 flex flex-wrap gap-2">
                    {group.skills.map((skill) => (
                      <span className="rounded-full border border-white/10 bg-white/[0.045] px-3 py-1.5 text-sm text-white/62 transition group-hover:border-cyan-300/30 group-hover:text-white" key={skill}>
                        {skill}
                      </span>
                    ))}
                  </div>
              ...
              <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
                {techStack.map((tech, index) => (
                  <motion.div
                    ...
                    whileHover={{ y: -6, borderColor: "rgba(103,232,249,0.45)" }}
                  >
                    {tech}
                  </motion.div>
                ))}
          ...
          <SectionReveal id="github" className="mx-auto max-w-7xl px-4 py-28 sm:px-6 lg:px-8">
            <div className="mb-12">
              <p className="eyebrow">GitHub Activity</p>
              <h2 className="section-title mt-5 max-w-4xl">Dynamic repository cards, language signals, and an animated contribution field.</h2>
            </div>
            <GitHubActivity />
          </SectionReveal>
          ...
          <SectionReveal className="mx-auto max-w-7xl px-4 py-28 sm:px-6 lg:px-8">
            <div className="mb-12">
              ...
              <h2 className="section-title mt-5 max-w-4xl">Signals from the work, written without the usual portfolio theater.</h2>
            </div>
            <div className="grid gap-5 md:grid-cols-2">
              {testimonials.map((item) => (
                <article className="panel p-7" key={item.name}>
                  <p className="text-2xl leading-9 text-white/78">&ldquo;{item.quote}&rdquo;</p>
                  <div className="mt-8 border-t border-white/10 pt-5">
                    <strong className="block text-white">{item.name}</strong>
                    <span className="mt-1 block text-sm text-white/42">{item.role}</span>
                  </div>
                </article>
              ))}
            </div>
          ...
          <SectionReveal className="mx-auto max-w-7xl px-4 py-28 sm:px-6 lg:px-8">
            <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr]">
              <div>
                ...
                <h2 className="section-title mt-5">Recent work, open-source proof, and technical trajectory.</h2>
              </div>
              <div className="relative">
                <div className="absolute bottom-0 left-4 top-0 w-px bg-gradient-to-b from-cyan-300/0 via-cyan-300/50 to-fuchsia-300/0" />
                {experience.map((item) => (
                  <article className="relative mb-8 pl-12" key={`${item.title}-${item.period}`}>
                    ...
                    <span className="text-sm uppercase tracking-[0.22em] text-cyan-100/60">{item.period}</span>
                    <h3 className="mt-3 text-2xl font-semibold text-white">{item.title}</h3>
                    <p className="mt-1 text-sm text-fuchsia-100/60">{item.company}</p>
                    <p className="mt-4 leading-7 text-white/55">{item.detail}</p>
                  </article>
                ))}
              </div>
            </div>
          ...
          <SectionReveal id="contact" className="mx-auto max-w-7xl px-4 py-28 sm:px-6 lg:px-8">
            <div className="panel grid gap-10 overflow-hidden p-6 sm:p-8 lg:grid-cols-[0.85fr_1.15fr]">
              <div>
                <p className="eyebrow">Contact</p>
                <h2 className="section-title mt-5">Let’s build something fast, observable, and dependable.</h2>
        ...
        <footer className="relative z-10 border-t border-white/10 px-4 py-10 sm:px-6 lg:px-8">
          <div className="mx-auto flex max-w-7xl flex-col justify-between gap-4 text-sm text-white/42 md:flex-row">
            <span>© 2026 Harsheet Sharma. Crafted with Next.js, GSAP, Lenis, and Framer Motion.</span>
  ...
  return (
    ...
    >
      <motion.div className="text-center" animate={{ opacity: loaded ? 0 : 1 }}>
        ...
        <p className="text-sm uppercase tracking-[0.45em] text-white/55">Initializing portfolio</p>
      </motion.div>
    </motion.div>
  );
}
...
function InteractiveBackdrop() {
  ...
  return (
    <div ref={ref} className="pointer-events-none fixed inset-0 z-0">
      ...
      <div className="absolute left-[-12%] top-[12%] size-[28rem] rounded-full bg-cyan-400/10 blur-3xl" data-parallax="70" />
      <div className="absolute bottom-[18%] right-[-10%] size-[30rem] rounded-full bg-fuchsia-400/10 blur-3xl" data-parallax="110" />
    </div>
  );
}
    ...
    >
      ...
      <div className="relative rounded-3xl border border-white/10 bg-black/45 p-5">
        ...
        <div className="font-mono text-sm leading-7 text-white/64">
          <p><span className="text-cyan-200">$</span> engineer.profile --cinematic</p>
          <p>name: <span className="text-white">{profile.name}</span></p>
          <p>role: <span className="text-white">{profile.role}</span></p>
          <p>focus: <span className="text-white">queues, APIs, reliability</span></p>
          <p>location: <span className="text-white">{profile.location}</span></p>
          <p>status: <span className="text-emerald-200">available immediately</span></p>
        </div>
      </div>
      <div className="relative mt-4 grid gap-4 sm:grid-cols-2">
        {metrics.slice(0, 2).map((metric) => (
          <div className="rounded-3xl border border-white/10 bg-white/[0.035] p-5" key={metric.label}>
            <strong className="text-3xl font-black">{metric.value}</strong>
            <span className="mt-2 block text-sm leading-5 text-white/45">{metric.label}</span>
          </div>
        ))}
      </div>
    </motion.aside>
...
function ProjectCard({ project, index }: { project: (typeof projects)[number]; index: number }) {
  return (
    ...
    >
      <div className={cn("absolute inset-0 bg-gradient-to-br opacity-70 transition duration-500 group-hover:opacity-100", project.gradient)} />
      ...
      <div className="relative">
        ...
        <h3 className="mt-8 text-4xl font-black leading-tight text-white">{project.title}</h3>
        <p className="mt-5 text-lg leading-8 text-white/66">{project.summary}</p>
      ...
      <div className="relative">
        <div className="mb-6 flex flex-wrap gap-2">
          {project.stack.map((item) => (
            <span className="rounded-full border border-white/15 bg-black/20 px-3 py-1.5 text-sm text-white/72" key={item}>
              {item}
            </span>
          ))}
        </div>
        <div className="flex gap-3">
          ...
          <a className="icon-link" href={project.repo} target="_blank" rel="noreferrer" aria-label={`${project.title} GitHub`}>
            <GitBranch className="size-5" />
          </a>
        </div>
      </div>
